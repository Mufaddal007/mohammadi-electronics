...
$KEY_PATH = "C:/Users/INDIA TECHNOLOGY/Downloads/pi_zero_w.ppk"
$SSH_KEY_PATH = "C:\Users\INDIA TECHNOLOGY\Downloads\pi_zero_w_openssh.ppk"
...
ssh -i "$SSH_KEY_PATH" "$PI_USER@$PI_IP" "mkdir -p $REMOTE_STAGING_PATH && rm -rf $REMOTE_STAGING_PATH/*"
...
.\pscp.exe -i "$KEY_PATH" -r -pw "1999" "$LOCAL_DIST_PATH\*" $RemoteDestination
...
$DeployCommand = "sudo find $REMOTE_TARGET_PATH -mindepth 1 -maxdepth 1 ! -name 'media' -exec rm -rf {} + && sudo cp -r $REMOTE_STAGING_PATH/* $REMOTE_TARGET_PATH/ && sudo chown -R mufaddal:www-data $REMOTE_TARGET_PATH && sudo chmod -R 755 $REMOTE_TARGET_PATH"

ssh -i "$SSH_KEY_PATH" "$PI_USER@$PI_IP" $DeployCommand
